...
def load_global_path(filename='town04_max30_start1.pkl'):
    """저장된 global path 로드"""
...
class MPCController:
    ...
    def __init__(self, vehicle, global_path=None, config=None):
        ...
        self.max_steer_angle = config.get('max_steer_angle',1.0)
    ...
    def get_dynamic_kappa_limits(self, velocity):
        ...
        velocity = max(velocity, 2.0)   # 핵심: 저속 보호
        # print(velocity)
    ...
    def step(self, waypoints):
        ...
        if success:
            ...
            # Curvature → Steering angle
            # target_steering = np.arctan(curvature * self.wheel_base)
            # target_steering = np.clip(target_steering, -self.max_steer_angle, self.max_steer_angle)
            ...
            # control.steer = float(smoothed_steering / self.max_steer_angle)
