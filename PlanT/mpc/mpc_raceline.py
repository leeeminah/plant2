...
class MPCRaceController:
    ...
    def get_reference_trajectory(self, waypoints):
        ...
        for i in range(self.horizon + 1):
            ...
            ref_traj[3, i] = ( wp['velocity'] / 3.6 ) * self.velocity_scale
            # print(wp['velocity'])
    ...
    def setup_visualization(self):
        ...
        # ==================== 전체 Raceline 표시 ====================
        # 전체 raceline을 배경으로 표시
        raceline_xs = [wp['x'] for wp in self.raceline]
        raceline_ys = [wp['y'] for wp in self.raceline]
        self.raceline_plot, = self.ax.plot(
            raceline_xs, raceline_ys, 
            'gray', 
            linewidth=1.5, 
            alpha=0.4, 
            label='Full Raceline',
        ...
        self.trajectory_plot, = self.ax.plot([], [], 'b-', linewidth=2, label='History', alpha=0.7, zorder=3)
        self.optimal_path_plot, = self.ax.plot([], [], 'r--', linewidth=3, label='MPC Path', alpha=0.8, zorder=4)
        self.waypoints_plot, = self.ax.plot([], [], 'g*', markersize=10, label='Lookahead', alpha=0.6, zorder=2)

        # ==================== 전역 범위 설정 ====================
        margin = 50.0  # m
        self.ax.set_xlim(min(raceline_xs) - margin, max(raceline_xs) + margin)
        self.ax.set_ylim(min(raceline_ys) - margin, max(raceline_ys) + margin)
        # =======================================================
...
def main():
    ...
    # raceline, metadata = load_raceline('routes/town04_raceline_shortest.pkl')
    ...
    try:
        ...
        mpc_config = {
            ...
            'horizon': 15,
            'dt': 0.1,
            'Q': [100.0, 100.0, 50.0, 30.0],      # ← 속도 weight 증가
            'R': [0.5, 5.0],                      # ← 가속 penalty 감소
            'Qf': [200.0, 200.0, 100.0, 150.0],
            'a_min': -10.0,                       # ← 강화
            'a_max': 10.0,                        # ← 강화
            'kappa_min': -0.25,                   # ← 증가
            'kappa_max': 0.25,                    # ← 증가
